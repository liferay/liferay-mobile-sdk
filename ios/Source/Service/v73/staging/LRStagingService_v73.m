...
#import "LRStagingService_v73.h"

/**
 * @author Bruno Farache
 */
@implementation LRStagingService_v73
...
- (BOOL)hasRemoteLayoutWithUuid:(NSString *)uuid groupId:(long long)groupId privateLayout:(BOOL)privateLayout error:(NSError **)error {
	...
	NSDictionary *_command = @{@"/staging/has-remote-layout": _params};

	return [self boolValue:(NSNumber *)[self.session invoke:_command error:error]];
}

- (void)propagateExportImportLifecycleEventWithCode:(int)code processFlag:(int)processFlag processId:(NSString *)processId arguments:(NSArray *)arguments error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"code": @(code),
		@"processFlag": @(processFlag),
		@"processId": [self checkNull: processId],
		@"arguments": [self checkNull: arguments]
	}];

	NSDictionary *_command = @{@"/staging/propagate-export-import-lifecycle-event": _params};
