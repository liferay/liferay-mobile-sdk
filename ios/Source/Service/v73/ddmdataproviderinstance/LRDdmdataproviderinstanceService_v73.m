...
#import "LRDdmdataproviderinstanceService_v73.h"

/**
 * @author Bruno Farache
 */
@implementation LRDdmdataproviderinstanceService_v73
...
- (NSNumber *)getDataProviderInstancesCountWithCompanyId:(long long)companyId groupIds:(NSArray *)groupIds error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"companyId": @(companyId),
		@"groupIds": [self checkNull: groupIds]
	}];

	NSDictionary *_command = @{@"/ddm.ddmdataproviderinstance/get-data-provider-instances-count": _params};

	return (NSNumber *)[self.session invoke:_command error:error];
}

- (NSDictionary *)fetchDataProviderInstanceByUuidWithUuid:(NSString *)uuid error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"uuid": [self checkNull: uuid]
	}];

	NSDictionary *_command = @{@"/ddm.ddmdataproviderinstance/fetch-data-provider-instance-by-uuid": _params};
...
- (NSDictionary *)getDataProviderInstanceByUuidWithUuid:(NSString *)uuid error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"uuid": [self checkNull: uuid]
	}];

	NSDictionary *_command = @{@"/ddm.ddmdataproviderinstance/get-data-provider-instance-by-uuid": _params};

	return (NSDictionary *)[self.session invoke:_command error:error];
}

- (NSArray *)getDataProviderInstancesWithCompanyId:(long long)companyId groupIds:(NSArray *)groupIds start:(int)start end:(int)end error:(NSError **)error {
	NSMutableDictionary *_params = [NSMutableDictionary dictionaryWithDictionary:@{
		@"companyId": @(companyId),
		@"groupIds": [self checkNull: groupIds],
		@"start": @(start),
	...
	NSDictionary *_command = @{@"/ddm.ddmdataproviderinstance/get-data-provider-instances": _params};
