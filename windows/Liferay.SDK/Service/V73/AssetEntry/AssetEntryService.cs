...
namespace Liferay.SDK.Service.V73.AssetEntry
{
	public class AssetEntryService : ServiceBase
	{
		...
		public async Task<dynamic> IncrementViewCounterAsync(long companyId, string className, long classPK)
		{
			var _parameters = new JsonObject();
		...
		public async Task IncrementViewCounterAsync(JsonObjectWrapper assetEntry)
		{
			var _parameters = new JsonObject();

			this.MangleWrapper(_parameters, "assetEntry", "$languageUtil.getJSONWrapperClassName($parameter.type)", assetEntry);
		...
		public async Task<dynamic> GetEntryAsync(string className, long classPK)
		{
			var _parameters = new JsonObject();
